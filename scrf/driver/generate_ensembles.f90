...
! -----------------------------------------------------------------------------
  ...
  ! temporary values
  real(DP),dimension(12) :: clen_month_tmean= (/1058.53, 1216.52, 1137.26, 962.64, 895.36, 763.74, 629.39, 650.82, 996.21, 1129.24, 1250.76, 1152.05/)
  real(DP),dimension(12) :: clen_month_prcp= (/290.25, 278.77, 246.90, 219.92, 183.17, 143.70, 111.21, 119.47, 183.84, 241.29, 263.62, 284.26/)
  real(DP),dimension(12) :: clen_month_trange= (/344.97, 352.29, 349.27, 378.64, 366.32, 346.70, 295.97, 291.39, 388.96, 483.01, 403.84, 333.49/)
  ! add by TGQ
...
!            climo_precip = climo_precip / real(month_days(current_month),kind(sp))
        ...
        if (grid%elv(isp1, isp2) .gt.-300.0) then
          ...
          if (cprob .lt. (1.0_dp-real(pop(isp1, isp2, istep), kind(dp)))) then 
           ...
           & istep), kind(dp))
            ...
            elseif(trim(time_mode) .eq. 'daily' .or. trim(time_mode) .eq. 'DAILY') then
              ...
              ra =  real(pcp(isp1,isp2,istep),kind(dp))+rn*real(pcp_error(isp1,isp2,istep),kind(dp))*1.5
          ...
          elseif(trim(time_mode) .eq. 'daily' .or. trim(time_mode) .eq. 'DAILY') then
            !TMEAN
            ! Add by TGQ: 
            ra = real(tmean(isp1,isp2,istep),kind(dp)) + real(tmean_random(isp1,isp2),kind(dp))*real(tmean_error(isp1,isp2,istep),kind(dp))*1.5 
            ...
            !trange
            ! Add by TGQ
            ra = real(trange(isp1,isp2,istep),kind(dp)) + real(trange_random(isp1,isp2),kind(dp))*real(trange_error(isp1,isp2,istep),kind(dp))*1.5
